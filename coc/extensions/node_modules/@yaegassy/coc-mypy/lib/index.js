...
var require_pump = __commonJS({
  "node_modules/pump/index.js"(exports, module2) {
    ...
    var isFS = function(stream2) {
      ...
      if (!fs5)
        return false;
      return (stream2 instanceof (fs5.ReadStream || noop) || stream2 instanceof (fs5.WriteStream || noop)) && isFn(stream2.close);
    };
    var isRequest2 = function(stream2) {
...
var require_fd_slicer = __commonJS({
  "node_modules/fd-slicer/index.js"(exports) {
    ...
    util3.inherits(FdSlicer, EventEmitter);
    ...
    util3.inherits(BufferSlicer, EventEmitter);
...
var require_yauzl = __commonJS({
  "node_modules/yauzl/index.js"(exports) {
    ...
    function open(path4, options, callback) {
      ...
      fs5.open(path4, "r", function(err, fd) {
        if (err)
          return callback(err);
    ...
    util3.inherits(ZipFile, EventEmitter);
    ...
    util3.inherits(RandomAccessReader, EventEmitter);
    ...
    util3.inherits(RefUnrefFilter, PassThrough2);
...
var require_extract_zip = __commonJS({
  "node_modules/extract-zip/index.js"(exports, module2) {
    ...
    var { createWriteStream, promises: fs5 } = require("fs");
    ...
    var pipeline2 = promisify(stream2.pipeline);
    var Extractor = class {
  ...
  "node_modules/rimraf/rimraf.js"(exports, module2) {
    ...
    var rmkids = (p, options, cb) => {
      ...
      options.readdir(p, (er, files) => {
        ...
        let errState;
        files.forEach((f) => {
          rimraf2(path4.join(p, f), options, (er2) => {
            if (errState)
              return;
            if (er2)
...
// package.json
var vscodeMypyVersion = "v2023.6.0";

// src/constant.ts
var EXTENSION_NS = "mypy-type-checker";
var UPSTREAM_NAME = "vscode-mypy";
var VSCODE_MYPY_VERSION = vscodeMypyVersion;
...
function getPythonEnvPath() {
  let pythonPath = "";
  pythonPath = import_which.default.sync("python3", { nothrow: true }) || "";
  if (pythonPath) {
    return pythonPath;
  }
  pythonPath = import_which.default.sync("python", { nothrow: true }) || "";
  if (pythonPath) {
    return pythonPath;
  }
  return pythonPath;
}
function getPythonBultinInstallPath() {
  let pythonPath = import_coc.workspace.getConfiguration(EXTENSION_NS).get("builtin.pythonPath", "");
  if (pythonPath) {
    return pythonPath;
  }
  pythonPath = import_which.default.sync("python3", { nothrow: true }) || "";
...
async function existsPythonImportModule(pythonPath, moduleName) {
...
function getMypyLspMypyPath(context) {
  let toolPath = void 0;
  if (import_fs.default.existsSync(import_path.default.join(context.storagePath, "vscode-mypy", "venv", "Scripts", "mypy.exe")) || import_fs.default.existsSync(import_path.default.join(context.storagePath, "vscode-mypy", "venv", "bin", "mypy"))) {
    if (process.platform === "win32") {
      toolPath = import_path.default.join(context.storagePath, "vscode-mypy", "venv", "Scripts", "mypy.exe");
    } else {
      toolPath = import_path.default.join(context.storagePath, "vscode-mypy", "venv", "bin", "mypy");
    }
  }
  return toolPath;
}
function getMypyLspServerInterpreterPath(context) {
  let pythonCommandPath = void 0;
  if (import_fs.default.existsSync(import_path.default.join(context.storagePath, "vscode-mypy", "venv", "Scripts", "python.exe")) || import_fs.default.existsSync(import_path.default.join(context.storagePath, "vscode-mypy", "venv", "bin", "python"))) {
    if (process.platform === "win32") {
      pythonCommandPath = import_path.default.join(context.storagePath, "vscode-mypy", "venv", "Scripts", "python.exe");
    } else {
      pythonCommandPath = import_path.default.join(context.storagePath, "vscode-mypy", "venv", "bin", "python");
    }
  }
  return pythonCommandPath;
}
function getMypyLspServerScriptPath(context) {
  let serverScriptPath = void 0;
  if (import_fs.default.existsSync(import_path.default.join(context.storagePath, "vscode-mypy", "bundled", "tool", "lsp_server.py"))) {
    serverScriptPath = import_path.default.join(context.storagePath, "vscode-mypy", "bundled", "tool", "lsp_server.py");
  }
...
async function createLanguageClient(context) {
  const devServerInterpreter = import_coc2.workspace.expand(
    import_coc2.workspace.getConfiguration(EXTENSION_NS).get("dev.serverInterpreter", "")
  );
  const devServerScript = import_coc2.workspace.expand(
    import_coc2.workspace.getConfiguration(EXTENSION_NS).get("dev.serverScript", "")
  );
  ...
  if (!serverInterpreter || !serverScript)
    return;
  const serverOptions = {
  ...
  const client2 = new import_coc2.LanguageClient(EXTENSION_NS, "mypy-lsp", serverOptions, clientOptions);
  return client2;
}
...
function convertFromWorkspaceConfigToInitializationOptions() {
  const settings = import_coc2.workspace.getConfiguration(EXTENSION_NS);
  const initializationOptions = {
    globalSettings: {
      cwd: import_coc2.workspace.root,
      workspace: import_coc2.Uri.parse(import_coc2.workspace.root).toString(),
      args: settings.get("args"),
      severity: settings.get("severity", DEFAULT_SEVERITY),
      path: settings.get("path"),
      ignorePatterns: settings.get("ignorePatterns", []),
      ...
      showNotifications: settings.get("showNotifications", "off"),
      extraPaths: [],
      reportingScope: settings.get("reportingScope", "file"),
      preferDaemon: settings.get("preferDaemon", true)
    },
    settings: {}
  };
...
async function getInitializationOptions(context) {
  ...
  if (import_coc2.workspace.getConfiguration(EXTENSION_NS).get("interpreter", []).length === 0) {
  ...
  if (!import_coc2.workspace.getConfiguration(EXTENSION_NS).get("useDmypy")) {
    if (initializationOptions.globalSettings.path.length === 0) {
      const envMypyCommandPath = import_which2.default.sync("mypy", { nothrow: true });
      if (envMypyCommandPath) {
        ...
        if (mypyPath)
          initializationOptions.globalSettings.path = [mypyPath];
      }
    }
  }
  return initializationOptions;
...
var pipeline = import_util2.default.promisify(import_stream2.default.pipeline);
...
function register(context, client2) {
  context.subscriptions.push(
    import_coc3.commands.registerCommand(`${EXTENSION_NS}.installServer`, handleInstallServer(context, client2))
  );
}
function handleInstallServer(context, client2) {
  return async () => {
    const msg = `Install/Upgrade ${UPSTREAM_NAME}'s language server?`;
    const ret = await import_coc3.window.showPrompt(msg);
    if (ret) {
      if (client2) {
        await client2.stop();
      }
      ...
      import_coc3.commands.executeCommand("editor.action.restart");
    } else {
    }
  };
}
async function doDownload(context) {
  ...
  const _path = import_path2.default.join(context.storagePath, `${UPSTREAM_NAME}.zip`);
  const randomHex = (0, import_crypto.randomBytes)(5).toString("hex");
  const tempFile = import_path2.default.join(context.storagePath, `${UPSTREAM_NAME}-${randomHex}.zip`);
  const destFileStream = import_fs2.default.createWriteStream(tempFile, { mode: 493 });
  ...
  await import_fs2.default.promises.unlink(_path).catch((err) => {
    if (err.code !== "ENOENT")
      throw err;
  });
  await import_fs2.default.promises.rename(tempFile, _path);
  statusItem.hide();
}
async function doExtract(context) {
  const zipPath = import_path2.default.join(context.storagePath, `${UPSTREAM_NAME}.zip`);
  const extractPath = import_path2.default.join(context.storagePath);
  const extractedFilenames = [];
  const targetPath = import_path2.default.join(context.storagePath, `${UPSTREAM_NAME}`);
  import_rimraf.default.sync(targetPath);
  if (import_fs2.default.existsSync(zipPath)) {
    await (0, import_extract_zip.default)(zipPath, {
      dir: extractPath,
    ...
    const extractedBasePath = import_path2.default.join(context.storagePath, extractedBaseDirName);
    const versionTxtFilePath = import_path2.default.join(extractedBasePath, "version.txt");
    import_fs2.default.writeFileSync(versionTxtFilePath, `tag: ${VSCODE_MYPY_VERSION}`);
    import_fs2.default.renameSync(extractedBasePath, targetPath);
    import_rimraf.default.sync(zipPath);
  }
}
async function doInstall(pythonCommand, context) {
  const requirementsTxtPath = import_path2.default.join(context.storagePath, UPSTREAM_NAME, "requirements.txt");
  const pathVenv = import_path2.default.join(context.storagePath, UPSTREAM_NAME, "venv");
  let pathVenvPython = import_path2.default.join(context.storagePath, UPSTREAM_NAME, "venv", "bin", "python");
  if (process.platform === "win32") {
    pathVenvPython = import_path2.default.join(context.storagePath, UPSTREAM_NAME, "venv", "Scripts", "python");
  }
  const statusItem = import_coc3.window.createStatusBarItem(0, { progress: true });
  statusItem.text = `Installing mypy language server in progress...`;
  statusItem.show();
  const installCmd = `"${pythonCommand}" -m venv ${pathVenv} && ${pathVenvPython} -m pip install -r ${requirementsTxtPath}`;
  import_rimraf.default.sync(pathVenv);
  try {
    import_coc3.window.showInformationMessage(`Installing mypy language server in progress...`);
    await exec2(installCmd);
    statusItem.hide();
    import_coc3.window.showInformationMessage(`Installation of mypy language server is now complete!`);
  } catch (error) {
    statusItem.hide();
    import_coc3.window.showErrorMessage(`Installation of mypy language server failed. | ${error}`);
    throw new Error();
  }
}
...
function register2(context, client2) {
  context.subscriptions.push(
    import_coc4.commands.registerCommand(`${EXTENSION_NS}.restart`, async () => {
      if (client2) {
        if (client2.serviceState !== import_coc4.ServiceStat.Stopped) {
          await client2.stop();
        }
      }
      client2.start();
...
async function register3(context, client2) {
  await client2.onReady();
  context.subscriptions.push(
    import_coc5.commands.registerCommand(`${EXTENSION_NS}.showOutput`, () => {
...
function register4(context) {
  context.subscriptions.push(
    import_coc6.commands.registerCommand(`${EXTENSION_NS}.version`, async () => {
      const versionTextPath = import_path3.default.join(context.storagePath, UPSTREAM_NAME, "version.txt");
      if (import_fs3.default.existsSync(versionTextPath)) {
        const version = import_fs3.default.readFileSync(versionTextPath, { encoding: "utf8" });
        import_coc6.window.showInformationMessage(version);
      }
    })
  );
}

// src/features/showDocumentation.ts
var import_coc7 = require("coc.nvim");
async function register5(context, client2) {
  await client2.onReady();
  if (!import_coc7.workspace.getConfiguration(EXTENSION_NS).get("showDocumantaion.enable", true))
    return;
  const documentSelector = [{ scheme: "file", language: "python" }];
  context.subscriptions.push(
    import_coc7.languages.registerCodeActionProvider(
      documentSelector,
      new ShowDocumentationCodeActionProvider(client2),
      EXTENSION_NS
    )
  );
}
var ShowDocumentationCodeActionProvider = class {
  constructor(client2) {
    this.source = "Mypy";
    this.client = client2;
  }
  async provideCodeActions(document2, range, context) {
    const doc = import_coc7.workspace.getDocument(document2.uri);
    const wholeRange = import_coc7.Range.create(0, 0, doc.lineCount, 0);
    let whole = false;
    ...
    if (this.lineRange(range) && context.diagnostics.length > 0) {
      ...
      if (line && line.length) {
        const mypyRuleContents = [];
        context.diagnostics.forEach((d) => {
          var _a;
          if (d.source === this.source) {
            if ("codeDescription" in d) {
              const mypyDiagnostic = d;
              if ((_a = mypyDiagnostic.codeDescription) == null ? void 0 : _a.href) {
...
async function activate(context) {
  if (!import_coc8.workspace.getConfiguration(EXTENSION_NS).get("enable"))
    return;
  const extensionStoragePath = context.storagePath;
  if (!import_fs4.default.existsSync(extensionStoragePath)) {
    import_fs4.default.mkdirSync(extensionStoragePath, { recursive: true });
  }
  ...
  if (!client)
    return;
  context.subscriptions.push(import_coc8.services.registLanguageClient(client));
  register(context, client);
  register2(context, client);
  register3(context, client);
  register4(context);
  register5(context, client);
  if (import_coc8.workspace.getConfiguration(EXTENSION_NS).get("useDmypy", true)) {
    import_coc8.workspace.registerAutocmd({
      event: "VimLeavePre",
      request: true,
      callback: async () => {
        if (client) {
